## Purpose: Summarize fire deverity in vegetation, ecoregion, fire event categories, TSLF categories,
...
# break TSLF into categories
d = d %>%
  mutate(TSLF_cat = cut(TSLF,breaks=c(-1,0,5,15,30,50,1000),labels=c("0","1-5","6-15","16-30","31-50","50+"), include.lowest=TRUE))
...
sig_tslf = unique(phs_by_tslf$TSLF_cat)

datatable(phs_by_tslf,options=list(pageLength=100, lengthChange=FALSE, dom='t', columnDefs =          list(list(className = 'dt-center', targets = "_all"))), width=500)
```

### Percent high severity by CWHR and TSLF
Only including CWHR-TSLF combinations with > 1000 points
```{r}
phs_by_cwhr_and_tslf = d %>%
  filter(cwhr_gp %in% sig_cwhr) %>%
  group_by(cwhr_gp,TSLF_cat) %>%
  summarize(phs = sum(rdnbr_h)/n(),
            n_pts = n()) %>%
  mutate(phs = round(phs,2)) %>%
  filter(n_pts > 1000) %>%
...
# make it wide
phs_wide = phs_by_cwhr_and_tslf %>%
  mutate(across(everything(),as.character)) %>%
  pivot_wider(names_from=TSLF_cat,values_from=phs,values_fill = list(phs="--"))

datatable(phs_wide,options=list(pageLength=500, lengthChange=FALSE, dom='t', columnDefs =          list(list(className = 'dt-center', targets = "_all"))), width=500)
```


### Percent high severity by Fire
Including only fires with > 500 points
```{r}
phs_by_fire = d %>%
  group_by(FIRE_NA) %>%
  summarize(phs = sum(rdnbr_h)/n(),
...
sig_fire = unique(phs_by_fire$FIRE_NA)

datatable(phs_by_fire,options=list(pageLength=100, lengthChange=FALSE, dom='t', columnDefs =          list(list(className = 'dt-center', targets = "_all"))), width=500)
```